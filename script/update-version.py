...
table = (
    ...
    , ( 'CMakeLists.txt'
        , r'set\W+jthread_lite_version\W+"([0-9]+\.[0-9]+\.[0-9]+)"\W+$'
        , 'set( jthread_lite_version "{major}.{minor}.{patch}" )\n' )

#    , ( 'example/cmake-pkg/CMakeLists.txt'
#        , r'set\W+jthread_lite_version\W+"([0-9]+\.[0-9]+(\.[0-9]+)?)"\W+$'
#        , 'set( jthread_lite_version "{major}.{minor}" )\n' )
#
#    , ( 'script/install-xxx-pkg.py'
#        , r'\jthread_lite_version\s+=\s+"([0-9]+\.[0-9]+\.[0-9]+)"\s*$'
#        , 'jthread_lite_version = "{major}.{minor}.{patch}"\n' )
    ...
    , ( 'include/nonstd/jthread.hpp'
        , r'\#define\s+jthread_lite_MAJOR\s+[0-9]+\s*$'
        , '#define jthread_lite_MAJOR  {major}' )

    , ( 'include/nonstd/jthread.hpp'
        , r'\#define\s+jthread_lite_MINOR\s+[0-9]+\s*$'
        , '#define jthread_lite_MINOR  {minor}' )

    , ( 'include/nonstd/jthread.hpp'
        , r'\#define\s+jthread_lite_PATCH\s+[0-9]+\s*$'
        , '#define jthread_lite_PATCH  {patch}\n' )
)
